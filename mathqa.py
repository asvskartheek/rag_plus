...
def evaluate_mathqa(solver):
    test_data = load_dataset("allenai/math_qa")["test"] #.select(range(10))

    y_hat = []
    y_true = []
    for item in tqdm(test_data, total=len(test_data)):
        res = solver(problem=item["Problem"],
    ...
    df.to_csv("mathqa_predictions.csv", index=False)
...
class MathQABaseline(Signature):
    ...
    reasoning = OutputField(description="The step-by-step reasoning to solve the problem.")
    answer = OutputField(description="The correct option among a-d. Just the letter, no other text.")
